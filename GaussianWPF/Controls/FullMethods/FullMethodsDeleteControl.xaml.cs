...
/// <summary>
/// Interaction logic for FullMethodsDeleteControl.xaml
/// </summary>
public partial class FullMethodsDeleteControl : UserControl, INotifyPropertyChanged
{
	private readonly ILogger<FullMethodsDeleteControl> _logger;
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="FullMethodsDeleteControl"/> class with dependency injection.
	/// </summary>
	...
	public FullMethodsDeleteControl(ILogger<FullMethodsDeleteControl> logger, IFullMethodsEndpoint fullMethodsEndpoint, ISpinStatesElectronicStatesMethodFamiliesEndpoint spinStatesElectronicStatesMethodFamiliesEndpoint, IBaseMethodsEndpoint baseMethodsEndpoint)
	{
		_logger = logger;
	...
	/// </summary>
	public event EventHandler<ChildControlEventArgs<FullMethodsDeleteControl>>? ChildControlEvent;

	/// <summary>
	/// Gets or sets the Full Method view model being deleted.
	/// </summary>
	/// <remarks>
	...
	/// <summary>
	/// Gets or sets the identifier of the Full Method to delete.
	/// </summary>
	/// <remarks>
	...
	protected void OnPropertyChanged([CallerMemberName] string? propertyName = null)
	{
		...
		{
			_logger.LogDebug("{UserControl} {Method} called with CallerMemberName = {PropertyName}.", nameof(FullMethodsDeleteControl), nameof(OnPropertyChanged), propertyName);
		}
		...
		if (_logger.IsEnabled(LogLevel.Debug))
		{
			_logger.LogDebug("{UserControl} {Method} returning.", nameof(FullMethodsDeleteControl), nameof(OnPropertyChanged));
		}
	}
	...
	protected override void OnInitialized(EventArgs e)
	{
		if (_logger.IsEnabled(LogLevel.Debug))
		{
			_logger.LogDebug("{UserControl} {EventHandler} called with {EventArgs}.", nameof(FullMethodsDeleteControl), nameof(OnInitialized), e);
		}

		base.OnInitialized(e);
		DataContext = this;
		PropertyChanged += FullMethodsDeleteControl_PropertyChanged;

		if (_logger.IsEnabled(LogLevel.Debug))
		{
			_logger.LogDebug("{UserControl} {EventHandler} returning.", nameof(FullMethodsDeleteControl), nameof(OnInitialized));
		}
	}

	private void FullMethodsDeleteControl_PropertyChanged(object? sender, PropertyChangedEventArgs e)
	{
		if (_logger.IsEnabled(LogLevel.Debug))
		{
			_logger.LogDebug("{UserControl} {EventHandler} called with {Sender} and {EventArgs}.", nameof(FullMethodsDeleteControl), nameof(FullMethodsDeleteControl_PropertyChanged), sender, e);
		}
		...
		if (e.PropertyName is nameof(FullMethodId))
		{
			...
			catch (HttpIOException ex)
			{
				...
				{
					_logger.LogError(ex, "{UserControl} {EventHandler} called with {Sender} and {EventArgs} had an error.", nameof(FullMethodsDeleteControl), nameof(FullMethodsDeleteControl_PropertyChanged), sender, e);
				}
			}
		...
		if (_logger.IsEnabled(LogLevel.Debug))
		{
			_logger.LogDebug("{UserControl} {EventHandler} returning.", nameof(FullMethodsDeleteControl), nameof(FullMethodsDeleteControl_PropertyChanged));
		}
	}
	...
	private void DeleteButton_Click(object sender, RoutedEventArgs e)
	{
		try
		{
			if (_logger.IsEnabled(LogLevel.Debug))
			{
				_logger.LogDebug("{UserControl} {EventHandler} called with {Sender} and {EventArgs}.", nameof(FullMethodsDeleteControl), nameof(DeleteButton_Click), sender, e);
			}

			ErrorMessage = string.Empty;
			_fullMethodsEndpoint.DeleteAsync(FullMethodId).Wait();
			FullMethod!.LastUpdatedDate = DateTime.Now;
			FullMethod!.Archived = true;
			ChildControlEvent?.Invoke(this, new ChildControlEventArgs<FullMethodsDeleteControl>("delete", null));

			if (_logger.IsEnabled(LogLevel.Debug))
			{
				_logger.LogDebug("{UserControl} {EventHandler} returning.", nameof(FullMethodsDeleteControl), nameof(DeleteButton_Click));
			}
		}
		catch (HttpIOException ex)
		{
			...
			if (_logger.IsEnabled(LogLevel.Error))
			{
				_logger.LogError(ex, "{UserControl} {EventHandler} called with {Sender} and {EventArgs} had an error.", nameof(FullMethodsDeleteControl), nameof(DeleteButton_Click), sender, e);
			}
		}
		catch (AggregateException ae)
		{
	...
	private void BackToIndexButton_Click(object sender, RoutedEventArgs e)
	{
		if (_logger.IsEnabled(LogLevel.Debug))
		{
			_logger.LogDebug("{UserControl} {EventHandler} called with {Sender} and {EventArgs}.", nameof(FullMethodsDeleteControl), nameof(BackToIndexButton_Click), sender, e);
		}

		ChildControlEvent?.Invoke(this, new ChildControlEventArgs<FullMethodsDeleteControl>("index", null));

		if (_logger.IsEnabled(LogLevel.Debug))
		{
			_logger.LogDebug("{UserControl} {EventHandler} returning.", nameof(FullMethodsDeleteControl), nameof(BackToIndexButton_Click));
		}
	}
}
